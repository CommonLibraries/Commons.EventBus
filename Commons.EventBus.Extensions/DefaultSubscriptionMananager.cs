...
public class InMemorySubscriptionMananager : ISubscriptionMananager
{
    ...
    public string GetEventName<TEvent>()
        where TEvent : IEvent
    {
        return this.GetEventName(typeof(TEvent));
    }
    ...
    public IEnumerable<Subscription> GetSubscriptionsForEvent<TEvent>()
       where TEvent : IEvent
    {
        return this.GetSubscriptionsForEvent(typeof(TEvent));
    }

    public IEnumerable<Subscription> GetSubscriptionsForEvent(Type eventType)
    {
        var eventName = this.GetEventName(eventType);
    ...
    {
        var eventName = this.GetEventName(eventType);
        this.RemoveSubscription(eventType, eventHandlerType, eventName);
    }
    ...
    public void RemoveSubscription<TEvent, TEventHandler>()
        ...
        where TEventHandler : IEventHandler<TEvent>
    {
        this.RemoveSubscription(typeof(TEvent), typeof(TEventHandler));
    }
    ...
    public void RemoveSubscription<TEvent, TEventHandler>(string eventName)
       ...
       where TEventHandler : IEventHandler<TEvent>
    {
        this.RemoveSubscription(typeof(TEvent), typeof(TEventHandler), eventName);
    }

    public void RemoveSubscription(Type eventType, Type eventHandlerType, string eventName)
    {
        if (!this.subscribedEvents.ContainsKey(eventName))
        {
            return;
